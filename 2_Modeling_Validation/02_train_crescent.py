...
def main() -> None:
    ...
    csv_path = DATA_PROCESSED_DIR / "image_index_crescent.csv"
    if not csv_path.exists():
        raise FileNotFoundError("未找到 image_index_crescent.csv，请先运行 01_check_data.py")
    ...
    save_path = MODELS_DIR / "crescent_resnet18_best.pt"
    result = train_model(
        model,
        train_loader,
        val_loader,
    ...
    history_path = METRICS_DIR / "crescent_train_history.csv"
    ...
    with open(METRICS_DIR / "crescent_train_summary.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2)

    print("Crescent 模型训练完成：")
